    ...
    <main className={`min-h-screen ${isLight ? "bg-slate-50 text-slate-900" : "bg-slate-950 text-white"}`}>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-6 lg:px-10">
        ...
        <div className="mt-6 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-cyan-600">
              <FileText size={14} />{item.format}
            </span>
        ...
        <div className={`relative mt-8 overflow-hidden rounded-2xl border ${isLight ? "border-slate-200 bg-white" : "border-slate-700 bg-slate-900"}`}>
          {isPdfLoading && (
            <div className={`absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 ${isLight ? "bg-white/90 text-slate-700" : "bg-slate-900/90 text-slate-200"} backdrop-blur-sm transition-opacity duration-300`}>
              <Loader2 className="h-10 w-10 animate-spin text-cyan-500" />
              <p className="text-sm font-bold tracking-wide">Loading PDF Document...</p>
              <p className="text-xs text-slate-400 font-medium">Preparing viewer, please wait</p>
            </div>
          )}
          <iframe
            src={item.pdf}
            ...
            className="h-[calc(100vh-190px)] min-h-[680px] w-full"
          />
        </div>
      </section>
